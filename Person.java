...
public class Person extends ICICI {
	...
	{
		int min=158000;	int max=158999;
		this.setPerson_id((int)(Math.random() * (max - min + 1) + min));
		...
		else 
		{
			...
			{
				System.out.println("Wait till your ID is generated");
				this.setPerson_id((int)(Math.random() * (max - min + 1) + min));
				if(!list_person_ids.contains(this.getPerson_id())){
					list_person_ids.add(this.getPerson_id());
					break;	}
	...
	private void account_num_generator()
	{
		...
		else 
		{
			...
			{
				System.out.println("Wait till your Account Number is generated");
				this.setAccount_num((int)(Math.random() * (999999999L - 100000000L + 1) + 100000000L));
				if(!list_account_nums.contains(this.getAccount_num())){
					list_account_nums.add(this.getAccount_num());
					break;	}
	...
	private void transactions()
	{
		...
		while(trns)
		{
			...
			if (opt == 'd')
			{
				...
				this.deposit(Integer.parseInt(sc.nextLine()));
			}
			else if(opt == 'w')
			{
				System.out.println("Enter the amount to be withdrawn:");
				this.withdraw(Integer.parseInt(sc.nextLine()));
			}
			else if(opt == 'q')
			{
				trns =false;
	...
	private static void endnote() {
		...
		System.out.println("Have a pleasent day ahead. Take Care :)");
	}
